...
MSU::MSU(MSUDevice &_d, int _mod_len, mpz_t _modulus, mpz_t _r, mpz_t _r_inv)
    : device(_d) {
    ...
    mpz_inits(sq_in, sq_in_montgomery, modulus, r, r_inv, sq_out, NULL);
    mpz_set(modulus, _modulus);
    mpz_set(r, _r);
    mpz_set(r_inv, _r_inv);
    gmp_printf("Modulus is %Zd\n", modulus);
    gmp_printf("R is %Zd\n", r);
    gmp_printf("R_inv is %Zd\n\n", r_inv);
}
...
void MSU::compute_job() {
    ...
    // (sq_in * r) % n
    mpz_mul(sq_in_montgomery, sq_in, r);
    mpz_mod(sq_in_montgomery, sq_in_montgomery, modulus);

    // Perform the computation
    device.compute_job(t_start, t_final, sq_in_montgomery, sq_out);

    // convert from Montgomery representation
    // (sq_out * r_inv) % n
    mpz_mul(sq_out, sq_out, r_inv);
    mpz_mod(sq_out, sq_out, modulus);
